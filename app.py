...
def restart_program():
    ...
    try:
        ...
        for handler in p.open_files() + p.n_connections():
            try:
                os.close(handler.fd)
            except Exception as e:
    ...
    def sockf1(self, tok, online_ip, online_port, packet, key, iv):
        ...
        while True:
            try:
                ...
                logging.info(f"RAW PACKET: {data2.hex()}")
                #logging.info(data2)
...
[FFD700]👑 NAME : [FFFF00]Tom   
[FFD700]📌 Instagram : [00BFFF]@ig._tom_ 
[FF1493]╚══════════════════════════╝""", idinv
                            )
                    )
                    senthi = False
...
[FFD700][b][c]thank you for supporting follow ig Ig._tom_:[/b]
...
[FFD700]✨ Developer —͟͞͞ </> TOM  ⚡""",
                ...
                if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00]TOM  HERE [ffffff]BOT" in data:
                    pass
                else:
...
[FFA500]/🙃inv [id] -> [FFFFFF]Invite Any Player
[FFA500]/🙃join [team code]  -> [FFFFFF]join bot to Group
[FFA500]/🙃solo  -> [FFFFFF]leave bot from Group""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
                                    self.GenResponsMsg(
                                        f"""[C][B][FFFF00]───────────────
[C][B][FF8800] EMOTE COMMANDS
[C][B][FFFF00]───────────────

[FFA500]🙃@a [id] [emote id]
[FFFFFF]Attack with emote >> 
[FFA500]🙃@b [id] [emote id]""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
...
[00FF00]/🙃check [id] -> [FFFFFF]Check Ban Status""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
...
[00FF00]/🙃admin -> [FFFFFF]Know Bot's Admin""", uid
                                    )
                                )
                        except Exception as e:
...
# Command changed to /join tc, joins only once, does not leave
                if '1200' in data.hex()[0:4] and b'/join' in data:
                    try:
                        ...
                        clients.send(
                            self.GenResponsMsg(f"[C][B][32CD32]Attempting to team code: {room_id}", uid)
                        )
                        ...
                        clients.send(
                            self.GenResponsMsg(f"[C][B][00FF00]Successfully joined the group.", uid)
                        )
#ob51rio
                    except Exception as e:
